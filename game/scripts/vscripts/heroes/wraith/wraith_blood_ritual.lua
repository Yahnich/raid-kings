...
function wraith_blood_ritual:CastFilterResultTarget(target)
	if IsServer() then
		...
		if caster:HasTalent("wraith_blood_ritual_talent_1") then
			return UnitFilter(target, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_ALL, 0, caster:GetTeamNumber())
		else
			return UnitFilter(target, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, 0, caster:GetTeamNumber())
		end
	end
end
...
function wraith_blood_ritual:OnSpellStart()
	...
	if target:IsSameTeam(caster) then
		...
		for _, ally in ipairs( allies ) do
			...
			ParticleManager:FireRopeParticle("particles/heroes/wraith/wraith_lifestrike_heal/wraith_lifestrike_heal.vpcf", PATTACH_POINT_FOLLOW, caster, ally)
		end
