...
function wraith_life_strike:OnSpellStart()
	...
	local ProjectileHit = function(self, target, position)
		...
		if caster:GetHealth() + heal > caster:GetMaxHealth() then
			...
			local particle1 = ParticleManager:CreateParticle("particles/heroes/wraith/wraith_lifestrike_heal/wraith_lifestrike_heal.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
				...
				if spreadheal > 0 and ally ~= caster then
					...
					if ally:GetHealth() < ally:GetMaxHealth() then
						if ally:GetHealth() + spreadheal > ally:GetMaxHealth() then
							...
							local particle2 = ParticleManager:CreateParticle("particles/heroes/wraith/wraith_lifestrike_heal/wraith_lifestrike_heal.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
			...
			local particle = ParticleManager:CreateParticle("particles/heroes/wraith/wraith_lifestrike_heal/wraith_lifestrike_heal.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
